...
avgTSplit(w,1:size(graphPositions,2)/2) = 1/60*avgTimeSplit(graphPositions);
    ...
    repMin = [repMin mean(minimumAverageSplit(:,1:hMinnow),2,'omitnan')];
    repVel = [repVel mean(avgVelSplit(:,1:hMinnow),2,'omitnan')];
...
l = cell(1,4);
l{1}='Human Minnow'; l{2}='Human Shark'; l{3}='Robot Minnow'; l{4}='Robot Shark';    
legend(p,l,'Location','southoutside','Orientation','Horizontal');
text(7.8,9000,descr)
...
l = cell(1,4);
l{1}='Human Minnow'; l{2}='Human Shark'; l{3}='Robot Minnow'; l{4}='Robot Shark';    
legend(y,l,'Location','southoutside','Orientation','Horizontal');
text(7.8,30,descr)
...
l = cell(1,4);
l{1}='Human Minnow'; l{2}='Human Shark'; l{3}='Robot Minnow'; l{4}='Robot Shark';    
legend(u,l,'Location','southoutside','Orientation','Horizontal');
text(7.8,175,descr)
...
for k = 1:7
    for o = 1:21
      ...
      zTestMin(o,k) = (repMin(o,k)-yMinSpecies(k,1))/yMinStd(1,k);
      zTestVel(o,k) = (repVel(o,k)-yVelSpecies(k,1))/yVelStd(1,k);
        
    end
    ksResultMin = [ksResultMin kstest(zTestMin(:,k))];
    ksResultVel = [ksResultVel kstest(zTestVel(:,k))];
end